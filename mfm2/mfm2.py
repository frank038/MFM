...
class mainwindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        #
        # list of items to copy with copy/cut/paste and the action
        self.ITEMS_TO_COPY = None
        #
...
############# MOUSE functions ###############
    ...
    def populate_background_menu(self, mpop):
        ...
        rett = items_copy_clip.copyClip(self).on_cc_paste()
        if rett[0]:
            item0020.set_sensitive(True)
            item0020.connect("activate", self.on_paste)
            self.window.ITEMS_TO_COPY == rett[1]
...
################## copy/cut functions ##############
    ...
    def on_copy(self, widget, operation):
        ...
        if cc_list != "copy" or cc_list != "cut":
            ...
            try:
                rett = items_copy_clip.copyClip(self).on_cc_copycut()
                if rett[0] == False:
                    self.generic_dialog("Message", rett[1])
                elif rett[0] == 2:
                    self.window.ITEMS_TO_COPY = sent_items
            except Exception as e:
                self.generic_dialog("\n"+str(e), "")
...
################## past functions ##################
    ...
    def paste_operation(self):
        # 
        if self.window.ITEMS_TO_COPY == None:
            # get the data from the module
            rett = items_copy_clip.copyClip(self).on_cc_paste()
            # got data
            if rett[0] == 1:
                self.window.ITEMS_TO_COPY = rett[1]
        #
        if self.window.ITEMS_TO_COPY:
            items_copy.CopyDialog(self.working_path, self.window.ITEMS_TO_COPY, self.window, 0)
            ret = items_copy_clip.copyClip(self).on_cc_paste()
            if ret[0] in [0, 2]:
                # reset
                self.window.ITEMS_TO_COPY = None
...
#### DROP
    ...
    def on_drag_data_received(self, IV, context, x, y, selection, info, time):
        ...
        if len(DATA) > 0:
            ...
            dest_path_path = Gio.File.new_for_uri(DATA[0]).get_path()
            dest_path = os.path.dirname(dest_path_path)
            #
            if dest_path == self.working_path:
                # the target is a folder in the same directory
                ppath = IV.get_path_at_pos(x,y)
                # not the background
                if ppath:
                    # the drop is onto a folder
                    if self.modello[ppath][2] == "a":
                        folder_name = self.modello[ppath][1]
                        folder_path = os.path.join(self.working_path, folder_name)
                        #
                        if CON_ACT == 2:
                            CONACT = "copy"
                        elif CON_ACT == 4:
                        ...
                        items_copy.CopyDialog(folder_path, list_items, self.window, 0)
                        #
