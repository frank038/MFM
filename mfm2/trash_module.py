...
########################
    ...
    def on_selection_changed_trash(self, iconview):
        ...
        if len(iconview.get_selected_items()) == 1:
            ...
            dest_path_model = self.model[rrow][3]
            dest_path = self.get_dest_folder(dest_path_model)
            self.label302path.set_label("{}".format(dest_path))
    ...
    def get_dest_folder(self, dest_folder_temp):
        dest_folder = dest_folder_temp
        while (dest_folder != HOME):
    ...
    # restore the item from the trashcan
    def on_button_restore(self, button_restore):
        ...
        for rrow in rrows:
            ...
            #
            dest_folder = self.model[rrow][3]
            # if not writable the item will be restored into the home dir
            dest_folder = self.get_dest_folder(dest_folder)
            # if the destination folder doesnt exist anymore
            # try to make the missed folders or the item will be restored into home
            if not os.path.exists(dest_folder):
                try:
                    os.makedirs(dest_folder, 0o750, exist_ok=False)
                except Exception as E:
                    dest_folder = HOME
            #
            dst = os.path.join(dest_folder, self.model[rrow][1])
            #
            ...
            if os.path.exists(dst):
                ...
                ii = 1
                while ii:
                    if os.path.exists(os.path.join(dest_folder, nroot+"_("+str(ii)+")")):
                        ii += 1
                    else:
                        new_name_file = nroot+"_("+str(ii)+")"
                        ii = 0
                dst = os.path.join(dest_folder, new_name_file)
    ...
    # delete the item from the trashcan
    ...
    def ftrash_empty(trash_folder_files, trash_folder_info):
        ...
        for ffile in os.listdir(trash_folder_files):
            ...
            if os.path.islink(ffile_path) or os.path.isfile(ffile_path):
                try:
